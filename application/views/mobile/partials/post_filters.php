...
if(!empty($filters))
{
	...
	$selected_outlets = !empty($post_data['outlets']) ? explode(',',$filters[0]['outlets']) : [];
	$selected_tags = !empty($post_data['tags']) ? explode(',',$post_data['tags']) : [];
	$selected_statuses = !empty($post_data['statuses']) ? explode(',',$post_data['statuses']) : [];
}
?>
<form method="post" id="filter-form">

	<input type="hidden" name="statuses" id="statuses">
	<input type="hidden" name="outlets" id="outlets">
	...
	<div class="post-filters">
		...
		<div class="row">
			<div class="col-xs-6">
				<div class="form-group filter <?php echo in_array('posted', $selected_statuses) ? 'checked' : ''; ?>" data-status="posted" data-value="posted" data-group="post-status"><i class="tf-icon check-box circle-border border-black pull-xs-left"><i class="fa fa-check"></i></i> <label class="label-check-box" data-for=".f-posted">Posted</label></div>		
				...
				{
					?>
					<div class="form-group filter <?php echo in_array('draft', $selected_statuses) ? 'checked' : ''; ?>" data-status="draft" data-value="draft" data-group="post-status"><i class="tf-icon check-box circle-border border-black pull-xs-left"><i class="fa fa-check"></i></i> <label class="label-check-box" data-for=".f-draft">Draft</label></div>
					<?php
				}
				...
				{
					?>
					<div class="form-group filter <?php echo in_array('scheduled', $selected_statuses) ? 'checked' : ''; ?>" data-status="scheduled" data-value="scheduled" data-group="post-status"><i class="tf-icon check-box circle-border border-black pull-xs-left"><i class="fa fa-check"></i></i> <label class="label-check-box" data-for=".f-scheduled">Scheduled</label></div>
					<?php
				}
				?>
			</div>
			<div class="col-xs-6">
				...
				{
					?>
					<div class="form-group filter <?php echo in_array('pending', $selected_statuses) ? 'checked' : ''; ?>" data-status="pending" data-value="pending" data-group="post-status"><i class="tf-icon check-box circle-border border-black pull-xs-left"><i class="fa fa-check"></i></i> <label class="label-check-box" data-for=".f-pending">Pending</label></div>
					<?php
				}
				?>
