...
if(!empty($approved)){
	...
	<?php 
		foreach ($approved as $phase) {
			$name = '';
			if(!empty($phase->first_name)){
				$name = ucfirst($phase->first_name);
				if(!empty($phase->last_name)){
					$name = $name.' '.ucfirst(substr($phase->last_name, 0, 1));
				}
			}
			?>
			<li class="approved">
				...
				<?php
					...
					if(file_exists(upload_path().$this->user_data['img_folder'].'/users/'.$phase->user_id.'.png'))
					{
						$image_path = upload_url().$this->user_data['img_folder'].'/users/'.$phase->user_id.'.png';
					}
				?>
