...
if(!function_exists('get_summary'))
{
    function get_summary($brand_id,$date = '')
    {
        $CI = & get_instance();   
        $CI->load->model('post_model');     
        return $CI->post_model->get_summary($brand_id,$date);
    }
}
...
if(!function_exists('get_approval_list_buttons'))
{
    function get_approval_list_buttons($post,$deadline,$phase_status,$user_group,$approver_status,$phase_id,$brand_id,$is_any_pending_approver,$edit_req_required = 1)
    {
        $CI = & get_instance(); 
        $html_to_return = '';
        if((check_user_perm($CI->user_id,'approve',$brand_id) OR ($CI->user_id == $CI->user_data['account_id']) OR $user_group == 'Master Admin') AND !empty($phase_status))
        {
