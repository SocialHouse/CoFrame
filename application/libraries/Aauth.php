...
class Aauth {
	...
	public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL) {
		...
		*/
		...
		if ($query->num_rows() > 0 && $this->config_vars['ddos_protection'] && ! $this->update_login_attempts($row->email)) {
			...
			$this->error('You have exceeded your login attempts, your account has now been locked.');
			return FALSE;
		}
		...
		if ($query->num_rows() > 0) {
			$this->error('Your account has not been verified. Please check your e-mail and verify your account.');
			return FALSE;
		}
		...
		if($this->config_vars['recaptcha_active']){
			if( ($this->config_vars['use_cookies'] == TRUE && $this->CI->input->cookie('reCAPTCHA', TRUE) == 'true') || ($this->config_vars['use_cookies'] == FALSE && $this->CI->session->tempdata('reCAPTCHA') == 'true') ){
				...
				if(!$resp->success){
					$this->error('Sorry, the reCAPTCHA text entered was incorrect.');
					return FALSE;
				}
			}
		}
	 	...
	 	if($this->config_vars['totp_active'] == TRUE AND $this->config_vars['totp_only_on_ip_change'] == FALSE){
			...
			if ($query->num_rows() > 0 AND !$totp_code) {
				$this->error('Authentication Code required');
				return FALSE;
			}else {
	...
	public function control( $perm_par = FALSE ){
		...
		if ( ! $this->is_allowed($perm_id) OR ! $this->is_group_allowed($perm_id) ){
			if( $this->config_vars['no_permission'] ) {
				$this->error('Sorry, you do not have access to the resource you requested.');
				redirect($this->config_vars['no_permission']);
			}
			else {
				echo 'Sorry, you do not have access to the resource you requested.';
				die();
			}
		}
	}
	 ...
	 */
	public function create_user($email, $pass, $name = FALSE,$user_data = []) {
		...
		if ($this->user_exist_by_name($name) && $name != FALSE) {
			$this->error("Account already exists on the system with that username.  Please enter a different username, or if you forgot your password, please click the link below");
			$valid = FALSE;
		}
	...
	public function update_user($user_id, $email = FALSE, $pass = FALSE, $name = FALSE) {
		...
		if ($name != FALSE) {
			if ($this->user_exist_by_name($name)) {
				$this->error("Username already exists on the system.  Please enter a different username.");
				$valid = FALSE;
			}
	...
	public function create_group($group_name, $definition = '') {
		...
		$this->info('Group name already exists');
		return FALSE;
	}
	...
	public function add_member($user_id, $group_par,$brand_id = NULL,$parent_id = NULL) {
		...
		$this->info('User is already member of group');
		return TRUE;
	}
	...
	public function send_pm( $sender_id, $receiver_id, $title, $message ){
		...
		if ( !is_numeric($receiver_id) OR $sender_id == $receiver_id ){
			$this->error('It is not possible to send a Message to yourself.');
			return FALSE;
		}
	...
	public function get_pm($pm_id, $set_as_read = TRUE){
		...
		if ($query->num_rows() < 1) {
			$this->error( 'Private Message not found' );
			return FALSE;
		}
	...
	//get matching permission by given string
	function check_user_perm($user_id,$perm,$brand_id = NULL,$account_id = NULL)
	{
		...
		if($account_id){
			$this->aauth_db->where('parent_id', $account_id);
		}
