...
class Tumblr {
	...
	function get_request_token($oauth_callback = NULL) 
	{
		...
		if (!empty($oauth_callback)) {
			$parameters['oauth_callback'] = $this->ci->config->item('auth_callback');
	...
	// 	}
	// 	if($this->ci->session->userdata('tumblr_access_token') && $this->ci->session->userdata('tumblr_access_token_secret'))
	// 	{
	// 		return;
	// 	}
	// 	else
	...
	// 		return $this->get_request_token(base_url($this->callback_url));
	// 	}
	// }
	...
	// 			$this->ci->session->unset_userdata('tumblr_request_token');
	// 			$this->ci->session->unset_userdata('tumblr_request_token_secret');
	...
	{
		$this->ci->session->unset_userdata('tumblr_access_token');
		$this->ci->session->unset_userdata('tumblr_access_token_secret');
		$this->ci->session->unset_userdata('tumblr_request_token');
		$this->ci->session->unset_userdata('tumblr_request_token_secret');
	}
